 ...
 */
public class L777OutputReader extends DefaultChemObjectReader {

    public static final String BOND_LENGTH = "org.openscience.cdk.io.L777OutputReader:bond.length";
    // energy is in hartrees: Eh = 4.359 744 34 x 10-18 J
    public static final String STRUCTURE_ENERGY = "org.openscience.cdk.io.L777OutputReader:structure.energy";
    public static final String OPTIMIZATION_CYCLES = "org.openscience.cdk.io.L777OutputReader:optimization.cycles";
    public static final String MAGNETIC_TENSOR = "org.openscience.cdk.io.L777OutputReader:isotropic.magnetic.tensor";
    public static final String COUPLING_CONSTANTS = "org.openscience.cdk.io.L777OutputReader:isotropic.coupling.constants";
    public static final String NORMAL_TERMINATION = "org.openscience.cdk.io.L777OutputReader:normal.termination";
    ...
    // NMR tables
    private final static Pattern REGEX_NMR_PARAMETERS_START = Pattern.compile("NMR Shielding including "+
            "vibrational correction");
    protected final static Pattern REGEX_NMR_PARAMETERS_STOP = Pattern.compile("GradGradGradGradGradGradGrad");

    protected final static Pattern REGEX_SS_COUPLING_FLAG = Pattern.compile("J \\(Hz\\) including the "+
            "vibrational correction");
...
//            "\\s+(\\d+)\\s+(\\w+)\\s+Isotropic\\s=\\s+(-?\\d+\\.\\d{10}D-?\\+?\\d{2})");
    ...
    private static ILoggingTool logger =
            LoggingToolFactory.createLoggingTool(L777OutputReader.class);

    public L777OutputReader(Reader reader) {
        input = new BufferedReader(reader);
    }

    public L777OutputReader(InputStream input) {
        this(new InputStreamReader(input));
    }

    public L777OutputReader() {
...
//                            readPartialCharges(model);
                ...
                if (REGEX_NORMAL_TERMINATION.matcher(lastLine).find())
                    model.setProperty(L777OutputReader.NORMAL_TERMINATION, true);
